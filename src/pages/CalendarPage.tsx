...
import { ChevronLeft, ChevronRight, Plus, FolderKanban, ClipboardList } from 'lucide-react'
      ...
      <div className="rounded-2xl border border-border/60 bg-surface-1 overflow-hidden">
        ...
        <div className="grid grid-cols-7">
          {calGrid.map((cell, i) => {
            ...
            const isOverdue = cell.date < todayStr && cell.isCurrentMonth
            ...
            return (
              <div
                ...
                className={`
                  relative flex flex-col gap-1 p-1.5 min-h-[90px] border-b border-r border-border/30 transition-all cursor-pointer
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <span className={`
                    w-6 h-6 flex items-center justify-center rounded-full text-[11px] font-body font-600
                    ${cell.isToday ? 'bg-teal-500 text-white' : isSelected ? 'text-teal-400' : 'text-text-secondary'}
                  `}>
                    {new Date(cell.date + 'T12:00:00').getDate()}
                  </span>

                  {/* Quick-create buttons — shown on hover */}
                  {cell.isCurrentMonth && (
                    <div
                      className="hidden group-hover:flex items-center gap-0.5 opacity-0 hover:opacity-100 transition-opacity"
                    ...
                    return (
                      <span
                        ...
                        className={`text-[9px] font-body font-500 px-1.5 py-0.5 rounded w-full truncate leading-tight ${
                          t.column === 'completed' ? 'bg-emerald-400/10 text-emerald-400/70 line-through' : `${pCfg.bg} ${pCfg.color}`
                        }`}
                      >
                        {t.title}
                      </span>
                ...
                {/* Add buttons — shown at bottom on hover */}
                {cell.isCurrentMonth && (
                  <div
                    className="absolute bottom-1 right-1 hidden group-hover:flex items-center gap-0.5"
                    style={{ display: 'none' }}
                    onClick={(e) => e.stopPropagation()}
                  >
                  </div>
                ...
                {/* Overdue indicator */}
                {isOverdue && dayTasks.some((t) => t.column !== 'completed') && (
